from utils.utils import Target, Rectangle, cost_matrix
    ...
    def initialize(self, img, detections):
        if len(detections) > 0:
            ...
            for det in detections:
                ...
                idx+=1
            self.persistent_weights = np.ones((self.particles_batch * len(detections)), dtype = float) * ( 1/float(self.particles_batch))
    ...
    def estimate(self, img = None, draw = False, color = (0, 255, 0)):
        ...
        if num_estimated > 0:
            ...
            for et in estimated_targets:
                target = Target(Rectangle(et[0], et[1], et[0] + et[2], et[1] + et[3]), random.randint(0,1000),\
